...
class plantillas_wni_invoice(models.Model):
    ...
    def _get_serie(self):
        cr = self.env.cr
        if self.origin:
            sql = "select splot.name from stock_picking pi left join stock_pack_operation sp on sp.picking_id =pi.id left join stock_pack_operation_lot spl on spl.operation_id=sp.id left join stock_production_lot splot on splot.id=spl.lot_id where  pi.origin='"+str(self.origin)+"' AND sp.product_id="+str(self.product_id.id)+" AND splot.name != 'None'"
        else:
